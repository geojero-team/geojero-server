...
--   제목은 무엇을 보는가만(버스 이야기 · 「유일」 · 「가장」 · 원천에 없는 말 없이), 소개는 150자 이하(대표 코스는 110자 이하 — 아래 「밤」).
...
--   ── 밤: 대표 소개 열 개를 카드 6줄 안으로 다시 썼다(2026-09-17 밤 사용자 — 「본문 너무 긴 곳들이 몇 개 있어 최대 6줄로」) ──
--   150자 글이 카드에서 7~8줄이었다(② 6-01 8줄 · ⑤ 3-11 7줄). CSS 로 자르면 문장이 끊기므로 글을 줄였다 — 110자 이하,
--   렌더 실측 근거는 CourseDataTest 「대표코스_소개는_카드_여섯_줄_안에_드는_110자_이하다」. ⑤ 3-11 은 V36(운영)의 글이라 아래 4) 에서 UPDATE 한다.
--   남긴 것: 무엇을 보는가 · 거제시 코스면 그 사실 한 번(「거제시 ○일코스 가운데 N곳을 원문 순서대로」) · 방문 순서 흐름.
--   뺀 것: 「고현터미널을 거쳐」(버스 이야기는 코스를 고를 이유가 아니다) · 「전망·명소로 분류한 세 곳」류(카드 분류 배지가 말한다) ·
--   ⑤ 의 「55번으로」 · 「돌아가는 버스를 기다리며」 · 「도보 1분 거리」(판매처 상품 문구라 우리 말로 쓰지 않는다 — 디자인브리프 부록 G).
--   새로 쓴 말의 근거(같은 날 운영 /api/pois/{id} TourAPI 소개문): 외도보타니아 「식물의 천국이라는 뜻의 해상 식물 공원」 ·
--   바람의언덕 「도장포 마을이 나오고, 그 마을 북쪽에 자리 잡은 곳이 바로 바람의 언덕」 · 맹종죽 「대나무 중 가장 굵은 대나무 · 죽림」 ·
--   매미성 「돌을 하나하나 쌓고 … 홀로 성을 만들어 올렸다」 · 거제식물원 「돔 내부에는 300여 종 1만 주의 열대수목」 ·
--   양지암조각공원 「조각가의 작품 21점을 전시하는 예술공원」 · 거제현 관아 「조선시대 거제현의 일반 행정과 군사 업무를 총괄」.
--   ⑤ 「외도에서 두 시간 · 같은 선착장으로 돌아온다」는 ferry_courses(외도유람선 예약센터 원문, V24) · 기준문서 §3,
--   ⑥ 「네 곳 모두 거제 9경」 · ⑦ 「그중 세 곳」은 pois.nine_scenic_no(V28), ⑦ 「다섯 권역에서 한 곳씩」은 pois.region.
--   검토에서 고친 것: ② 6-01 이 「여섯 곳」이라 적고 다섯 곳만 불렀다(조선해양문화관이 빠지고 거제씨월드는 「지세포의 벨루가」뿐) —
--   여섯 곳을 방문 순서대로 모두 부르게 고쳤다(「굵은 대나무가 자라는」은 뺐다 — 제목 「굵은 대나무 숲」이 말한다).
--   CourseDataTest 「대표코스_소개는_스팟을_빠짐없이_방문_순서대로_말한다」가 지킨다.
...
COMMENT ON COLUMN courses.official_order_kept IS
  '이 코스의 방문 순서가 거제시 코스 원문 나열 순서와 같은가(2일 · 3일코스는 1일차 → 2일차 → 3일차를 이어 본다).';
...
FROM (VALUES
  (125, '학동몽돌해변 · 바람의언덕 · 해금강 · 조선해양문화관',
        ...
        '거제시 당일코스 가운데 네 곳을 원문 순서대로 봅니다. 파도가 몽돌을 굴리는 학동 해변, 풍차가 선 바람의언덕, '
        '바다의 금강산 해금강을 지나 배 만드는 원리를 체험하는 조선해양문화관에서 마칩니다.'),
...
-- ⑤ 3-11 소개 — V36(운영에 나감)의 150자 글이 카드에서 7줄이었다. V36 은 두고 여기서 고친다(위 제목 · 소개 절 「밤」).
UPDATE courses SET intro =
  '해상 식물 공원 외도보타니아는 배로만 갑니다. 도장포 선착장에서 외도상륙 유람선을 타고 외도에서 두 시간을 보낸 뒤 '
  '같은 선착장으로 돌아와 도장포 마을의 바람의언덕을 봅니다.'
WHERE course_code = '3-11';
