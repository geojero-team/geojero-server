...
class CourseDataTest {
   ...
   * 같은 날 저녁 배지에서 숫자를 뺐다(사용자 결정) — 초록 「거제시 추천 관광코스」 · 분류 「정원·숲」 · 주황 「거제 9경」.
  ...
  @Test void 대표코스_제목은_배지의_말을_되풀이하지_않고_두_줄_안이다() {
    var badgeWords = java.util.regex.Pattern.compile(
        "거제시|추천|관광코스|당일코스|2일코스|3일코스|원문|순서|곳|바다·해변|섬·유람선|전망·명소|정원·숲|역사·유적|전시·체험");
    for (var r : rows("""
  ...
  /**
   * ★ 대표 코스 소개는 카드에서 **6줄 안**이다(2026-09-17 밤 사용자 요청 — *"본문 너무 긴 곳들이 몇 개 있어 최대 6줄로"*).
   * 150자 상한(위 테스트)으로는 7~8줄이 나왔다 — 옛 ② 6-01(144자)이 사용자 화면에서 8줄 · ⑤ 3-11(150자)이 7줄이었다.
   * CSS 로 자르면 문장이 중간에 끊기므로 글을 짧게 다시 썼다(V37).
   *
   * 110자는 렌더 실측으로 정했다 — 코스 추천 카드 .intro(16/24 · word-break: keep-all), 헤드리스 크롬 390 폭:
   *   · 글 폭 308px(390 틀 그대로)에서 옛 글은 105~118자 5줄 · 121~136자 6줄 · 143~150자 7줄
   *   · 글 폭을 296px 로 좁히면 옛 글이 사용자 화면 줄 수(6-01 8줄 · 3-11 7줄)와 같아진다 — 거기서 118자부터 6줄 · 136자부터 7줄
   *   · 새 글은 가장 긴 109자(① 4-11)까지 **308px · 296px · 290px 모두 5줄 이하**
   * 띄어쓰기에서만 줄을 끊어 글자 수가 줄 수를 보장하지는 않는다 — 7줄이 처음 나온 136자보다 넉넉히 낮춰 110자로 둔다.
   */
  @Test void 대표코스_소개는_카드_여섯_줄_안에_드는_110자_이하다() {
    for (var r : rows("""
        SELECT course_code, intro FROM courses
        WHERE featured_rank IS NOT NULL ORDER BY featured_rank""")) {
      String intro = (String) r.get("intro");
      assertNotNull(intro, r.get("course_code") + ": 소개가 없다");
      int n = intro.codePointCount(0, intro.length());
      assertTrue(n <= 110, r.get("course_code") + ": 소개가 110자를 넘는다(" + n + "자) — 카드에서 6줄을 넘길 수 있다");
    }
  }
